---
title: "George_Tofaris_iPSC_DaNs_treated"
...
+ Stimulation : Non-seeded control(NSC), PD
...
+ Fragment Library type :ISR and IU
...
quant_dir <- "../kallisto_rfstranded/abundance/"
...
# samples metadata
samples_file <- "./June_DaN_treated_metadata.csv"
...
samples_meta <- read_csv(file = samples_file)
...
colnames(samples_meta)[8] <- "Tissue_type"
...
samples_meta_merged <- merge(samples_meta, quant_dirs_table, by.x="Sample_ID", by.y= "Sample", all.x=T)
...
samples_meta_print <- subset.data.frame(samples_meta_merged,select = c(Sample_ID,Replicate, Treatment, Treatment_time, percent_aligned,reads_aligned_millions, total_reads))
...
names(files_tximport) <- samples_meta[, "Sample_ID"]
...
saveRDS(txi, file="tximport_kallisto.rds")
```
...
txi = readRDS(file="./tximport_kallisto.rds")
rownames(samples_meta) <- samples_meta$Sample_ID


## if only protein coding genes are used:
## "transcripts missing genes: 56393"
stopifnot(all.equal(colnames(txi$counts), rownames(samples_meta)))

dds <- DESeqDataSetFromTximport(txi, samples_meta,design = ~ Replicate + Group)
...
if (pre_filter_type == "counts"){
  ...
  cat("Number of genes excluded: ", rowSums(fpkm_robust > expression_cutoff) < 3, fill=T)
...
write.table(sample_counts_norm, file="Tofaris_June21_iPSC_DaNS_DESEQ2_counts_normalised.txt", sep="\t", quote=F)
...
+ Rlog transformation tends to works well on small datasets (n<30)
...
#limma batch correct TPM_gt_1 and total features after pca
#corrected <- removeBatchEffect(logcounts(sce), covariates = test, design = design)
#assay(rld_1) <- limma::removeBatchEffect(assay(rld_1), covariates = test, design=design)
#limma_rld <- assay(rld_1)
#write.table(limma_rld, file="DeSEQ2_Mastroni_rlog_blinded_limma_corrected.txt", sep="\t", quote=F)
```
...
```{r sample_clustering_1, echo=FALSE}
library("pheatmap")
library("RColorBrewer")
sampleDistMatrix <- as.matrix( sampleDists )
rownames(sampleDistMatrix) <- paste(rld$Group,"_", rld$Replicate, sep="")
...
```{r Poisson_clustering_2, echo=FALSE}
samplePoisDistMatrix <- as.matrix( poisd$dd )
rownames(samplePoisDistMatrix) <- paste( dds$Group,"_",dds$Replicate, sep="" )
...
plot_data <- DESeq2::plotPCA(rld, intgroup =c("Replicate", "Group","Sample_ID","Treatment","Treatment_time"), returnData = T,ntop=500)
percent_var <- round(100 * attr(plot_data, "percentVar"), 2)

cat("top 500 most variable genes by RLog transformation:")
p <- ggplot(data = plot_data, aes(x=PC1, y=PC2, color= Treatment, shape= Treatment_time)) ## size = ?
...
plot_data <- DESeq2::plotPCA(rld, intgroup =c("Replicate", "Group","Sample_ID","Treatment","Treatment_time"), returnData = T,ntop=1000)
percent_var <- round(100 * attr(plot_data, "percentVar"), 2)

cat("top 1000 most variable genes by RLog transformation:")
p <- ggplot(data = plot_data, aes(x=PC1, y=PC2, color= Treatment_time, shape= Treatment)) ## size = ?
...
p <- p + geom_text_repel(aes(label=Replicate), size=2.5, point.padding = 0.5)
p <- p + xlab(paste0("PC1 (", percent_var[1], "%)"))
p <- p + ylab(paste0("PC2 (", percent_var[2], "%)"))
...
plot_data <- DESeq2::plotPCA(rld, intgroup =c("Replicate", "Group","Sample_ID","Treatment","Treatment_time"), returnData = T, ntop = dim(dds)[1])
percent_var <- round(100 * attr(plot_data, "percentVar"), 2)

p <- ggplot(data = plot_data, aes(x=PC1, y=PC2, color=Group, shape= Replicate))
...
```{r}
 plot_data <- plotPCA_higher_2_3(object = rld, intgroup =c("Replicate", "Group","Sample_ID","Treatment","Treatment_time"), returnData = T, ntop = 500)
percent_var <- round(100 * attr(plot_data, "percentVar"), 2)

cat("top 500 most variable genes by RLog transformation:")
p <- ggplot(data = plot_data, aes(x=PC2, y=PC3, color= Treatment, shape= Treatment_time)) ## size = ?
p <- p + geom_point(alpha = 0.6, size=8)
p <- p + geom_text_repel(aes(label=Sample_ID), size=2.5)
...
plot_data <- plotPCA_higher_2_3(object = rld, intgroup =c("Replicate", "Group","Sample_ID","Treatment","Treatment_time","Treatment"), returnData = T, ntop = 1000)
percent_var <- round(100 * attr(plot_data, "percentVar"), 2)

cat("top 1000 most variable genes by RLog transformation:")
p <- ggplot(data = plot_data, aes(x=PC2, y=PC3, color= Treatment_time, shape= Treatment )) ## size = ?
p <- p + geom_point(alpha = 0.9, size=6, position="jitter") + scale_shape(solid = FALSE)
p <- p + geom_text_repel(aes(label=Replicate), size=2.5, point.padding = 0.5) + scale_shape_manual(values=c(1, 0, 2,5))
...
plot_data <- plotPCA_higher_2_3(object = rld, intgroup =c("Replicate", "Group","Sample_ID","Treatment","Treatment_time"), returnData = T, ntop = dim(dds)[1])
percent_var <- round(100 * attr(plot_data, "percentVar"), 2)
...
```{r}
 plot_data <- plotPCA_higher_3_4(object = rld, intgroup =c("Replicate", "Group","Sample_ID","Treatment","Treatment_time"), returnData = T, ntop = 500)
percent_var <- round(100 * attr(plot_data, "percentVar"), 2)

cat("top 500 most variable genes by RLog transformation:")
p <- ggplot(data = plot_data, aes(x=PC3, y=PC4, color= Treatment, shape= Treatment_time)) ## size = ?
p <- p + geom_point(alpha = 0.6, size=8)
p <- p + geom_text_repel(aes(label=Sample_ID), size=2.5)
...
plot_data <- plotPCA_higher_3_4(object = rld, intgroup =c("Replicate", "Group","Sample_ID","Treatment","Treatment_time"), returnData = T, ntop = 1000)
percent_var <- round(100 * attr(plot_data, "percentVar"), 2)
...
plot_data <- plotPCA_higher_3_4(object = rld, intgroup =c("Replicate", "Group","Sample_ID","Treatment","Treatment_time"), returnData = T, ntop = dim(dds)[1])
percent_var <- round(100 * attr(plot_data, "percentVar"), 2)
...
```{r}
plot_data <- plotPCA_higher_5_6(object = rld, intgroup =c("Replicate", "Group","Sample_ID","Treatment","Treatment_time"), returnData = T, ntop = 500)
percent_var <- round(100 * attr(plot_data, "percentVar"), 2)

cat("top 500 most variable genes by RLog transformation:")
p <- ggplot(data = plot_data, aes(x=PC5, y=PC6, color= Treatment, shape= Treatment_time)) ## size = ?
p <- p + geom_point(alpha = 0.6, size=8)
p <- p + geom_text_repel(aes(label=Sample_ID), size=2.5)
...
plot_data <- plotPCA_higher_5_6(object = rld, intgroup =c("Replicate", "Group","Sample_ID","Treatment","Treatment_time"), returnData = T, ntop = 1000)
percent_var <- round(100 * attr(plot_data, "percentVar"), 2)
...
plot_data <- plotPCA_higher_5_6(object = rld, intgroup =c("Replicate", "Group","Sample_ID","Treatment","Treatment_time"), returnData = T, ntop = dim(dds)[1])
percent_var <- round(100 * attr(plot_data, "percentVar"), 2)
...
## PCA correlation with covariates 
 ...
 + using Rlog transformation and top 1000 HVG
...
```{r, fig.height=6, fig.width=10}
  ...
  meta <- samples_meta %>% 
    dplyr::select( -Clone, -Species, -Tissue_type,-path,-Group)
...
ggplot(gpca.dat, aes(x = dim1, y = dim2, color = Treatment, shape = Treatment_time)) + geom_text_repel(aes(label=Sample_name), size=2.5) +
...
* DEG log2FC threshold = 0.25

```{r}
## Week1: Treatments vs NSC

dds$Group <- relevel(dds$Group, ref="WK1_NSC")
...
res_Wk1_PD_NSC <- results(object=dds ,contrast = c("Group", "WK1_PD","WK1_NSC") , lfcThreshold = 0.25, alpha=alpha_cutoff)
...
```{r}
res_Wk1_PD_NSC_LFCshrink <- lfcShrink(dds = dds ,coef = "Group_WK1_PD_vs_WK1_NSC"  ,type="ashr")
DESeq2::plotMA(res_Wk1_PD_NSC_LFCshrink, ylim = c(-5,5))
...
res_print_Wk1_PD_NSC <- print_DESeq_table(res_Wk1_PD_NSC, t2g_info = t2g_genes, alpha_level = alpha_cutoff,logfcthreshold = 0)


    
    plot_up_genes <- res_print_Wk1_PD_NSC[fc == "up", target_id][1:top_genes]
    plot_up_genes <- plot_up_genes[!(is.na(plot_up_genes))] # if there are not top_genes DEGs, NA values will be returned
    plot_down_genes <- res_print_Wk1_PD_NSC[fc == "down", target_id][1:top_genes]
    plot_down_genes <- plot_down_genes[!(is.na(plot_down_genes))]
  
    
    
   res_print_Wk1_PD_NSC <-res_print_Wk1_PD_NSC[, gene_type := NULL] ## only protein coding genes
    res_print_Wk1_PD_NSC <- res_print_Wk1_PD_NSC[, target_id := NULL] ## remove "symbol - ensembl_ID"
    
    setnames(res_print_Wk1_PD_NSC, "desc", "___________long_gene_description___________")
   res_print_Wk1_PD_NSC <- res_print_Wk1_PD_NSC %>% drop_na()
```



```{r}
res_print_Wk1_PD_NSC %>% DT::datatable(caption="Week1:PD vs NSC", escape=F, extensions = 'Buttons', options = list(dom = 'Blfrtip', buttons = list('colvis', list(extend = 'copy', exportOptions = list(columns=':visible')), list(extend = 'csv', exportOptions = list(columns=':visible')), list(extend = 'excel', exportOptions = list(columns=':visible')))))
```
...
my_plot_counts(dds, plot_genes = plot_up_genes,title="upregulated genes in WK1: PD vs NSC", t2g_info = t2g_genes)
```

```{r, echo=FALSE, fig.height=9, fig.width=10}
my_plot_counts(dds, plot_genes = plot_down_genes,title="downregulated genes in WK1: PD vs NSC", t2g_info = t2g_genes)
```

* plot interesting genes 

```{r,echo=FALSE, fig.height=9, fig.width=10}
control_genes <- c("OPA1 - ENSG00000198836","MFN1 - ENSG00000171109","TOMM20 - ENSG00000173726","DNM1L - ENSG00000087470","TFAM - ENSG00000108064","NFE2L2 - ENSG00000116044","MTOR - ENSG00000198793","ARNT - ENSG00000143437","SNCA - ENSG00000145335","PARK7 - ENSG00000116288")
...
```{r, eval=FALSE}
interest_genes <- c("UBR5 - ENSG00000104517","USP7 - ENSG00000187555","TRIM28 - ENSG00000130726")
my_plot_counts(dds, plot_genes = interest_genes ,title="", t2g_info = t2g_genes)

```

```{r, eval=FALSE}
tpm <- txi$abundance
tpm_nsc <- as.data.frame(tpm) %>% select(contains("NSC"))
tpm_nsc <- tpm_nsc %>% filter(rownames(.) %in% interest_genes) %>% rownames_to_column("gene")

tpm_long <- tpm_nsc %>% gather(colnames(tpm_nsc)[2:7], key ="sample", value = "TPM_counts")
tpm_long$group <- c(rep("WK1_NSC",6), rep("WK2_NSC",6))

p <- ggplot(data = tpm_long, aes(x= group, y= TPM_counts, colour=group))
  p <- p + geom_violin(aes(group=group), alpha = 0, colour = "gray60", scale="width")
  ## p <- p + ggbeeswarm::geom_quasirandom(alpha = 0.6, size = 3, groupOnX = TRUE)
  p <- p + geom_point(position = position_jitterdodge(), size=5, alpha=0.6)
  p <- p + xlab(NULL)
  p <- p + facet_wrap(~gene, ncol= 2, scales="free_y",nrow = 1)
  p <- p + guides(colour = guide_legend(override.aes = list(alpha=1)))
  p <- p + ylab(" TPM Counts") + theme(legend.position="top")
  p <- p + theme(axis.text.x=element_text(angle=90, hjust=1, size=10)) + theme(axis.text.y = element_text(size=8))
  p
ggsave(filename = "2021_bulk_NSC_WK1_WK2_tpm_counts.tiff",width = 8, height =6, units = "in" )
```



```{r, eval =F}
# # Rlog Transformed normalised values
rlog_counts <- as.data.frame(norm_counts) %>% column_to_rownames("gene")%>% select(contains("NSC"))
rlog_nsc <- rlog_counts %>% filter(rownames(.) %in% interest_genes) %>% rownames_to_column("gene")

rlog_long <- rlog_nsc %>% gather(colnames(rlog_nsc)[2:7], key ="sample", value = "rlog_counts")
rlog_long$group <- c(rep("WK1_NSC",6), rep("WK2_NSC",6))

p <- ggplot(data = rlog_long, aes(x= group, y= rlog_counts, colour=group))
  p <- p + geom_violin(aes(group=group), alpha = 0, colour = "gray60", scale="width")
  ## p <- p + ggbeeswarm::geom_quasirandom(alpha = 0.6, size = 3, groupOnX = TRUE)
  p <- p + geom_point(position = position_jitterdodge(), size=5, alpha=0.6)
  p <- p + xlab(NULL)
  p <- p + facet_wrap(~gene, ncol= 2, scales="free_y",nrow = 1)
  p <- p + guides(colour = guide_legend(override.aes = list(alpha=1)))
  p <- p + ylab(" rlog normalised counts") + theme(legend.position="top")
  p <- p + theme(axis.text.x=element_text(angle=90, hjust=1, size=10)) + theme(axis.text.y = element_text(size=8))
  p
ggsave(filename = "2021_bulk_NSC_WK1_WK2_rlog_counts.tiff",width = 8, height =6, units = "in" )
```


```{r, eval = F}
# CPM based plots and values for genes of interest
cpm_counts <- fpm(dds, robust = T)
colsums <- colSums(cpm_counts)

cpm_counts <- as.data.frame(cpm_counts)
cpm_counts <- (cpm_counts) %>% dplyr::select(contains("NSC")) 
cpm_nsc_intgenes <- cpm_counts %>% filter(rownames(.) %in% interest_genes) %>% rownames_to_column("gene")

write.table(cpm_nsc_intgenes, file = "CPM_counts_Jan2021_WK1_WK2_NSC_samples_DaNS.txt", sep="\t", quote=F)

cpm_long <- cpm_nsc_intgenes %>% gather(colnames(cpm_nsc_intgenes)[2:7], key ="sample", value = "cpm_counts")
cpm_long$group <- c(rep("WK1_NSC",6), rep("WK2_NSC",6))

p <- ggplot(data = cpm_long, aes(x= group, y= cpm_counts, colour=group))
  p <- p + geom_violin(aes(group=group), alpha = 0, colour = "gray60", scale="width")
  ## p <- p + ggbeeswarm::geom_quasirandom(alpha = 0.6, size = 3, groupOnX = TRUE)
  p <- p + geom_point(position = position_jitterdodge(), size=5, alpha=0.6)
  p <- p + xlab(NULL)
  p <- p + facet_wrap(~gene, ncol= 2, scales="free_y",nrow = 1)
  p <- p + guides(colour = guide_legend(override.aes = list(alpha=1)))
  p <- p + ylab("CPM counts") + theme(legend.position="top")
  p <- p + theme(axis.text.x=element_text(angle=90, hjust=1, size=10)) + theme(axis.text.y = element_text(size=8))
  p
ggsave(filename = "2021_bulk_NSC_WK1_WK2_CPM_counts.tiff",width = 8, height =6, units = "in" )
```
...
write.table(resOrdered_WK1_PD_NSC_ens, file="deseq2_results_WK1_PD_vs._NSC.txt", sep="\t", quote=F)
...
library(EnhancedVolcano)
 Vol <- EnhancedVolcano(resOrdered_WK1_PD_NSC_ens,
    ...
    x = 'log2FoldChange',
    y = 'pvalue', subtitle = "", title = "WK1: PD vs. NSC", xlim = c(-13,26), labSize =3, FCcutoff = 0.25, axisLabSize = 6, titleLabSize = 8, legendLabSize = 6, pCutoff = 0.05, drawConnectors = F, captionLabSize = 6,
    widthConnectors = 0.2, colConnectors = 'grey30',caption = "Log2FC cutoff = 0.25; p-value cutoff=0.05" )
...
# #GO BP over-representation test plot 

```{r}
#GO BP over-representation test
scorePlot <- RScorePlot_v2(enrichGOResult_1, resOrdered_WK1_PD_NSC_ens, showCategory = 10, textSize = 9, title = "" ) + theme(title = element_text(size=8))
plot(scorePlot)
```
...
geneChangeList <-resOrdered_WK1_PD_NSC_ens_sig$entrez
#geneChangeList <- sort(geneChangeList, decreasing=TRUE)
#desiredGeneChangeList <- names(geneChangeList)[abs(geneChangeList) >1] 
 all_genes <- as.character(resOrdered_WK1_PD_NSC_ens$entrez)
enrich_Reactome <-  enrichPathway(gene = geneChangeList, pAdjustMethod = "BH", universe=all_genes, readable = T, organism = "human")
scorePlot <- RScorePlot_v2(enrich_Reactome, resOrdered_WK1_PD_NSC_ens, showCategory = 25, textSize = 9, title = "" ) + theme(title = element_text(size=8))
plot(scorePlot)
```
...
p1 <- ggplot(fgsea_reactome_tidy_top, aes(reorder(pathway, NES), NES)) +
  ...
  labs(x="Pathway", y="Normalized Enrichment Score",
       title="Top up and down Reactome pathways NES from GSEA") + theme(title=element_text(size=7))+theme(axis.text.y = element_text(size=7), legend.text = element_text(size=9), legend.title = element_text(size=8))+theme(plot.title = element_text(size=8))
p1
...
res_Wk2_PD_NSC <- results(object=dds ,contrast = c("Group", "WK2_PD","WK2_NSC") , lfcThreshold = 0.25, alpha=alpha_cutoff)
...
DESeq2::plotMA(res_Wk2_PD_NSC_LFCshrink, ylim = c(-3,3))
...
res_print_Wk2_PD_NSC <- print_DESeq_table(res_Wk2_PD_NSC, t2g_info = t2g_genes, alpha_level = alpha_cutoff,logfcthreshold = 0)
...
write.table(resOrdered_WK2_PD_NSC_ens, file="deseq2_results_WK2_PD_vs._NSC.txt", sep="\t", quote=F)
...
library(EnhancedVolcano)
 Vol <- EnhancedVolcano(resOrdered_WK2_PD_NSC_ens,
    ...
    x = 'log2FoldChange',
    y = 'pvalue', subtitle = "", title = "WK2: PD vs. NSC", xlim = c(-13,26), labSize =3, FCcutoff = 0.25, axisLabSize = 6, titleLabSize = 8, legendLabSize = 6, pCutoff = 0.01, drawConnectors = F, captionLabSize = 6,
    widthConnectors = 0.2, colConnectors = 'grey30',caption = "Log2FC cutoff = 0.25; p-value cutoff=0.01" )
 Vol <-   Vol +  ggplot2::coord_cartesian(xlim=c(-28, 26))  + ggplot2::scale_x_continuous(breaks=seq(-28,23, 5))
 Vol
```
...
resOrdered_WK2_PD_NSC_ens_sig <- resOrdered_WK2_PD_NSC_ens %>% filter(padj < 0.05) 

#geneChangeList <- resOrdered_Parent_stim_control_LFCshrink_ens_sig$log2FoldChange
geneChangeList <-resOrdered_WK2_PD_NSC_ens_sig$ens_gene
#geneChangeList <- sort(geneChangeList, decreasing=TRUE)
#desiredGeneChangeList <- names(geneChangeList)[abs(geneChangeList) >1] 
 all_genes <- as.character(resOrdered_WK2_PD_NSC_ens$ens_gene)
...
```{r, eval=F}
#GO BP over-representation test
scorePlot <- RScorePlot_v2(enrichGOResult_2, resOrdered_WK2_PD_NSC_ens, showCategory = 10, textSize = 9, title = "" ) + theme(title = element_text(size=8))
plot(scorePlot)
```
...
library(ReactomePA)
# only takes entrez id
geneChangeList <-resOrdered_WK2_PD_NSC_ens_sig$entrez
#geneChangeList <- sort(geneChangeList, decreasing=TRUE)
#desiredGeneChangeList <- names(geneChangeList)[abs(geneChangeList) >1] 
 all_genes <- as.character(resOrdered_WK2_PD_NSC_ens$entrez)
enrich_Reactome <-  enrichPathway(gene = geneChangeList, pAdjustMethod = "BH", universe=all_genes, readable = T, organism = "human")
scorePlot <- RScorePlot_v2(enrich_Reactome, resOrdered_WK2_PD_NSC_ens, showCategory = 25, textSize = 9, title = "" ) + theme(title = element_text(size=8))
plot(scorePlot)
```
...
```{r}
#pathways.gobp <- gmtPathways("./genesets/c5.bp.v7.1.symbols.gmt")
fgsea_gobp <- fgsea(pathways=pathways.gobp, stats=ranks, nperm=10000, maxSize = 500, minSize = 30)
# convert to tibble, sort by NES and select columns
fgsea_gobp_tidy <- fgsea_gobp %>% as_tibble() %>% arrange(padj) %>% dplyr::select(-pval, -ES, -nMoreExtreme)
# Show in a nice table:
...
## Overlap between Week 2 treatment DEGs

+ Upset plot

+ "distinct mode" - Under this mode, the seven combination sets are the seven partitions in the Venn diagram and they are mutually exclusive.

```{r}
library(ComplexHeatmap)
  
res_print_Wk1_PD_NSC <- as.data.frame(res_print_Wk1_PD_NSC)
rownames(res_print_Wk1_PD_NSC) <- res_print_Wk1_PD_NSC$ens_gene

res_print_Wk2_PD_NSC <- as.data.frame(res_print_Wk2_PD_NSC)
rownames(res_print_Wk2_PD_NSC) <- res_print_Wk2_PD_NSC$ens_gene



  ## Venn Diagram: DEGs for all 3 pairwise comparisons
  sig_wk1_PD_NSC_up <- rownames(res_print_Wk1_PD_NSC[res_print_Wk1_PD_NSC$fc == "up", ])
  sig_wk1_PD_NSC_down <- rownames(res_print_Wk1_PD_NSC[res_print_Wk1_PD_NSC$fc == "down", ])
  
   sig_wk2_PD_NSC_up <- rownames(res_print_Wk2_PD_NSC[res_print_Wk2_PD_NSC$fc == "up", ])
  sig_wk2_PD_NSC_down <- rownames(res_print_Wk2_PD_NSC[res_print_Wk2_PD_NSC$fc == "down", ])
...
## Overlap between Week 1 & 2 DEG genes 
...
library(ComplexHeatmap)
  ...
  WK_1_2_DEG <- list( sig_wk1_PD_NSC_up = sig_wk1_PD_NSC_up, sig_wk1_PD_NSC_down = sig_wk1_PD_NSC_down, sig_wk2_PD_NSC_up= sig_wk2_PD_NSC_up,
                    sig_wk2_PD_NSC_down = sig_wk2_PD_NSC_down)
  
  m1 = make_comb_mat(WK_1_2_DEG, mode = "distinct")
  #m1
```

---
title: "George_Tofaris_iPSC_pooled_PD_treatments_NSC version 2"
...
+ Pooled differential analysis for 9 PD replicates and NSC iPSC DaN samples
+ DEG comparisons : WK2 PD vs WK1 PD , WK2 NSC vs WK1 NSC , WK1 PD vs WK1 NSC, WK2 PD vs WK2 NSC
+ using ComBat-Seq for batch correction for PCA
...
+ plot heatmaps and PCA plots on ComBat batch corrected rld data
...
#mm <- model.matrix(~Group , colData(rld))
combat_rld <- sva::ComBat(rlog_matrix, batch = rld$Batch, mod =  mm, par.prior = T, ref.batch = 1, prior.plots = F)
...
## PCA correlation with covariates 
 ...
 + using  ComBat batch corrected Rlog transformed data (as massive batch effect)  and top 1000 HVG
...
# Differential Expression : WK2 PD vs WK1 PD
...
res_WK2PD_WK1PD <- results(object=dds ,contrast = c("Group", "WK2_PD","WK1_PD") , lfcThreshold = 0.50, alpha=0.05)
 mcols(res_WK2PD_WK1PD)
 res_summary <- capture.output(summary(res_WK2PD_WK1PD))
 cat(res_summary, sep="  \n* ")
 
 # number of differentially expressed genes based on cut off
 
 num_DEG <- sum(res_WK2PD_WK1PD$padj < alpha_cutoff & abs(res_WK2PD_WK1PD$log2FoldChange) >= 0.50, na.rm=TRUE)
    cat("  \n* Genes with adjusted P-value not NA (universe of genes): ", sum(!is.na(res_WK2PD_WK1PD$padj)))
    cat("  \n* Differentially expressed genes with log2FC >= 0.50 & p.adjusted <0.05: ", num_DEG, "  \n")
resOrdered_WK2PD_WK1PD <- res_WK2PD_WK1PD[order(res_WK2PD_WK1PD$padj),]
resOrdered_WK2PD_WK1PD<- as.data.frame(resOrdered_WK2PD_WK1PD )
...
```{r}
res_WK2PD_WK1PD_LFCshrink <- lfcShrink(dds = dds ,contrast = c("Group", "WK2_PD","WK1_PD") ,type="ashr")
DESeq2::plotMA(res_WK2PD_WK1PD_LFCshrink, ylim = c(-15,15))
resOrdered_WK2PD_WK1PD_LFCshrink <- res_WK2PD_WK1PD_LFCshrink[order(res_WK2PD_WK1PD_LFCshrink$padj),]
resOrdered_WK2PD_WK1PD_LFCshrink  <- as.data.frame(resOrdered_WK2PD_WK1PD_LFCshrink )
```
...
res_print_WK2PD_WK1PD <- print_DESeq_table(resOrdered_WK2PD_WK1PD_LFCshrink, t2g_info = t2g_genes, alpha_level = alpha_cutoff,logfcthreshold = 0.50)


    
    plot_up_genes <- res_print_WK2PD_WK1PD[fc == "up", target_id][1:top_genes]
    plot_up_genes <- plot_up_genes[!(is.na(plot_up_genes))] # if there are not top_genes DEGs, NA values will be returned
    plot_down_genes <-res_print_WK2PD_WK1PD[fc == "down", target_id][1:top_genes]
    plot_down_genes <- plot_down_genes[!(is.na(plot_down_genes))]
  
    
    
  res_print_WK2PD_WK1PD <- res_print_WK2PD_WK1PD[, gene_type := NULL] ## only protein coding genes
   res_print_WK2PD_WK1PD <- res_print_WK2PD_WK1PD[, target_id := NULL] ## remove "symbol - ensembl_ID"
    
    setnames(res_print_WK2PD_WK1PD, "desc", "___________long_gene_description___________")
   res_print_WK2PD_WK1PD <- res_print_WK2PD_WK1PD %>% drop_na()
```



```{r, fig.height=6}
res_print_WK2PD_WK1PD %>% DT::datatable(caption=" WK2 PD vs WK1 PD", escape=F, extensions = 'Buttons', options = list(dom = 'Blfrtip', buttons = list('colvis', list(extend = 'copy', exportOptions = list(columns=':visible')), list(extend = 'csv', exportOptions = list(columns=':visible')), list(extend = 'excel', exportOptions = list(columns=':visible')))))
```
...
my_plot_counts <- function(obj, plot_genes=NULL, intgroup_vec=c("Group", "Replicate", "Batch"), t2g_info=NULL, n_ncol=4, n_nrow =6, title=NULL){
  ...
  # plot_data_all[, gene_print := paste0(gene_idx, "  ", gene)]
  plot_data_all[, gene_print := paste0(gene_idx, "  ", ext_gene)] ## use only gene symbol as facet title
  plot_data_all <- plot_data_all %>% filter(Group %in% c("WK1_PD","WK2_PD"))
...
p <- my_plot_counts(dds, plot_genes = plot_up_genes,title="upregulated genes in WK2 PD vs WK1PD", t2g_info = t2g_genes, n_ncol = 5)
p
```

```{r, echo=FALSE, fig.height=9, fig.width=10}
my_plot_counts(dds, plot_genes = plot_down_genes,title="downregulated genes in WK2: PD vs WK1:PD", t2g_info = t2g_genes, n_ncol=5)
```
...
control_genes_sig <- resOrdered_WK2PD_WK1PD_LFCshrink %>% filter(rownames(.) %in% control_genes & padj<0.05 )
...
```{r, fig.height =9, fig.widht=10}
pdgwas_genes_sig <- resOrdered_WK2PD_WK1PD_LFCshrink %>% filter(rownames(.) %in% PD_risk_genes & padj<0.05 & log2FoldChange >= 0.5 )
my_plot_counts(dds, plot_genes = rownames(pdgwas_genes_sig),title=" GWAS PD genes UP at 5% FDR & 0.5 Log2FC", t2g_info = t2g_genes, n_ncol=4)
```
...
resOrdered_WK2PD_WK1PD_LFCshrink$gene <- rownames(resOrdered_WK2PD_WK1PD_LFCshrink)
resOrdered_WK2PD_WK1PD_LFCshrink_ens <- merge(resOrdered_WK2PD_WK1PD_LFCshrink, t2g_genes, by.all ="gene")
rownames(resOrdered_WK2PD_WK1PD_LFCshrink_ens) <- resOrdered_WK2PD_WK1PD_LFCshrink_ens$ens_gene
rm(resOrdered_WK2PD_WK1PD_LFCshrink)
resOrdered_WK2PD_WK1PD_LFCshrink_ens <- resOrdered_WK2PD_WK1PD_LFCshrink_ens[,c(1:7,11)]

# get rid of NAs for clusterprofiler
#library(tidyr)
resOrdered_WK2PD_WK1PD_LFCshrink_ens <- resOrdered_WK2PD_WK1PD_LFCshrink_ens %>% drop_na()

# map ids using annotation db
library(org.Hs.eg.db)
library(AnnotationDbi)

resOrdered_WK2PD_WK1PD_LFCshrink_ens$entrez <- mapIds(org.Hs.eg.db, keys= rownames(resOrdered_WK2PD_WK1PD_LFCshrink_ens), column = "ENTREZID", keytype = "ENSEMBL", multiVals = "first")
resOrdered_WK2PD_WK1PD_LFCshrink_ens$symbol <- mapIds(org.Hs.eg.db, keys= rownames(resOrdered_WK2PD_WK1PD_LFCshrink_ens), column = "SYMBOL", keytype = "ENSEMBL", multiVals = "first")
write.table(resOrdered_WK2PD_WK1PD_LFCshrink_ens, file="deseq2_results_Jan_Jun21_WK2_PD_vs WK1_PD_v2.txt", sep="\t", quote=F)
...
• 155  a-synuclein interacting proteins based on mass spec overlap with upregulated genes of which 2 are also a PD GWAS gene

• 94 a-synuclein interacting proteins based on mass spec overlap with downregulated genes 
...
```{r, echo=FALSE, eval=T}
  ...
  d <- "Overlap of WK2 PD vs WK1 PD DEG with PD related genes"
...
rownames(res_print_WK2PD_WK1PD) <- res_print_WK2PD_WK1PD$ens_gene
 ...
 # cairo_ps("./DEG_genes_Venn_diagram.eps", height = 4, width = 5, onefile = F)
   venn.plot <- venn.diagram(x=list(S1=as.matrix(sig_PD_up), S2=as.matrix(sig_PD_down), S3=as.matrix(PD_risk_genes_ens), S4=as.matrix(async_int_proteins_ens_v)), fill = c("red", "cyan3", "green3","yellow"), alpha=c(0.5, 0.5, 0.5,0.5), cex=0.6, cat.cex=0.55, fontfamily="sans", cat.fontfamily="sans", main.fontfamily = "sans", cat.dist=c(0.2,0.2,0.1,0.1),  filename=NULL, euler.d=F, scaled=F, print.mode=c("percent", "raw"), sigdigs = 2, category.names=paste0(c("WK2 PD UP", "WK2 PD Down","PD GWAS genes","a-synuclein interacting proteins"), "\n", c(length(sig_PD_up), length(sig_PD_down), length(PD_risk_genes_ens), length(async_int_proteins_ens_v))), main = d, main.cex = 0.7)
...
• heatmap showing Odds Ratio as colours and pvalue signficance at 5% FDR as * or NS

• OR > 1 = strong association between categories

• Overlap between alpha-synuclein interacting proteins and Upregulated proteins in WK2 PD vs WK1 PD  is NOT significant

```{r, warning=FALSE}
library(GeneOverlap)
deg_list <- list("WK2_PD_vs_WK1_PD_UP" = sig_PD_up , "WK2_PD_vs_WK1_PD_DN" = sig_PD_down)
pd_gene_l <- list("PD_GWAS_genes" = PD_risk_genes_ens, "a-sync_int_proteins" = async_int_proteins_ens_v)
gs.RNAseq =  as.numeric(length(rownames(resOrdered_WK2PD_WK1PD_LFCshrink_ens)))
...
library(EnhancedVolcano)
 Vol <- EnhancedVolcano(resOrdered_WK2PD_WK1PD_LFCshrink_ens,
    lab = resOrdered_WK2PD_WK1PD_LFCshrink_ens$symbol,
    x = 'log2FoldChange',
    y = 'pvalue', subtitle = "", title = "WK2:PD vs WK1:PD", xlim = c(-20,20), labSize =3, FCcutoff = 2.0, axisLabSize = 6, titleLabSize = 8, legendLabSize = 6, pCutoff = 0.001,pCutoffCol = 'padj', drawConnectors = F, captionLabSize = 6,
    widthConnectors = 0.2, colConnectors = 'grey30',caption = "Log2FC cutoff = 2.0; p-value cutoff= 0.01" )
...
resOrdered_WK2PD_WK1PD_LFCshrink_ens_sig <- resOrdered_WK2PD_WK1PD_LFCshrink_ens %>% filter(padj < 0.05 & abs(log2FoldChange) >= 0.5) 

#geneChangeList <- resOrdered_Parent_stim_control_LFCshrink_ens_sig$log2FoldChange
geneChangeList <- unique(rownames(resOrdered_WK2PD_WK1PD_LFCshrink_ens_sig))
#geneChangeList <- sort(geneChangeList, decreasing=TRUE)
#desiredGeneChangeList <- names(geneChangeList)[abs(geneChangeList) >1] 
 all_genes <- as.character(unique(rownames(resOrdered_WK2PD_WK1PD_LFCshrink_ens)))
...
scorePlot <- RScorePlot_v2(enrichGOResult_1, resOrdered_WK2PD_WK1PD_LFCshrink_ens, showCategory = 25, textSize = 9, title = "" ) + theme(title = element_text(size=8), axis.text = element_text(size=8)) + ggtitle("GO BP terms for WK2:PD vs WK1:PD")
plot(scorePlot)
```
...
geneChangeList <- unique(resOrdered_WK2PD_WK1PD_LFCshrink_ens_sig$entrez)
#geneChangeList <- sort(geneChangeList, decreasing=TRUE)
#desiredGeneChangeList <- names(geneChangeList)[abs(geneChangeList) >1] 
 all_genes <- as.character(unique(resOrdered_WK2PD_WK1PD_LFCshrink_ens$entrez))
enrich_Reactome <-  enrichPathway(gene = geneChangeList, pAdjustMethod = "BH", universe=all_genes, readable = T, organism = "human")
scorePlot <- RScorePlot_v2(enrich_Reactome, resOrdered_WK2PD_WK1PD_LFCshrink_ens, showCategory = 25, textSize = 9, title = "" ) + theme(title = element_text(size=8)) +ggtitle("Reactome terms for WK2:PD vs WK1:PD")
plot(scorePlot)
```
...
library(fgsea)
gsea_res_WK1PD_WK2PD <- resOrdered_WK2PD_WK1PD_LFCshrink_ens %>% dplyr::select(symbol,log2FoldChange) %>% na.omit() %>% distinct() %>% group_by(symbol) %>% summarize(log2FoldChange=mean(log2FoldChange))
# dim(gsea_res)
# head(gsea_res)

# fast Gene Set Enrichment Analysis
ranks<- deframe(gsea_res_WK1PD_WK2PD)
...
norm_counts_Ens_PD <- norm_counts_Ens %>%  dplyr::select(ends_with("PD"))
...
for (i in 1:5) {
  ...
  gene_counts = norm_counts_Ens_PD[match(leading_edge_ensg$ENSEMBL, rownames(norm_counts_Ens_PD)),]
...
res_WK2NSC_WK1NSC <- results(object=dds ,contrast = c("Group", "WK2_NSC","WK1_NSC") , lfcThreshold = 0.50, alpha=0.05)
...
```{r}
res_WK2NSC_WK1NSC_LFCshrink <- lfcShrink(dds = dds ,contrast = c("Group", "WK2_NSC","WK1_NSC") ,type="ashr")
...
my_plot_counts <- function(obj, plot_genes=NULL, intgroup_vec=c("Group", "Batch"), t2g_info=NULL, n_ncol=4, n_nrow =6, title=NULL){
  ...
  # plot_data_all[, gene_print := paste0(gene_idx, "  ", gene)]
  plot_data_all[, gene_print := paste0(gene_idx, "  ", ext_gene)] ## use only gene symbol as facet title
  plot_data_all <- plot_data_all %>% filter(Group %in% c("WK1_NSC","WK2_NSC"))
  p <- ggplot(data = plot_data_all, aes(x=Group, y=log2(count+1), colour=Group,shape=Batch))
  p <- p + geom_violin(aes(group=Group), alpha = 0, colour = "gray60", scale="width")
  ## p <- p + ggbeeswarm::geom_quasirandom(alpha = 0.6, size = 3, groupOnX = TRUE)
  p <- p + geom_point(position = position_jitterdodge(), size=5, alpha=0.6)
...
my_plot_counts(dds, plot_genes = rownames(pdgwas_genes_sig),title=" GWAS PD genes UP at 5% FDR & Log2FC 0.5", t2g_info = t2g_genes, n_ncol=4)
```
...
```{r, fig.height =8, fig.widht=10}
pdgwas_genes_sig <- resOrdered_WK2NSC_WK1NSC_LFCshrink %>% filter(rownames(.) %in% PD_risk_genes & padj<0.05 & log2FoldChange <= -0.5 )
my_plot_counts(dds, plot_genes = rownames(pdgwas_genes_sig),title=" GWAS PD genes Down at 5% FDR and Log2FC <= -0.5", t2g_info = t2g_genes, n_ncol=5)
```
...
write.table(resOrdered_WK2NSC_WK1NSC_LFCshrink_ens, file="deseq2_results_Jan_Jun_21_WK2_NSC_vsWK1_NSC_v2.txt", sep="\t", quote=F)
...
• 160  a-synuclein interacting proteins based on mass spec overlap with upregulated genes of which 2 are also a PD GWAS gene

• 128 a-synuclein interacting proteins based on mass spec overlap with downregulated genes 
...
• heatmap showing  Odds ratio as colours and pvalue signfiicance at 5% FDR as * or NS

• OR > 1 = strong association betweenc categories
...
## Evaluate Unique Up & Downs sets for PD (WK2 vs WK1)



```{r}
PD_UP_unique <- setdiff(sig_PD_up,sig_NSC_up)
PD_DN_unique <- setdiff(sig_PD_down, sig_NSC_down)

PD_unique <- c(PD_DN_unique,PD_UP_unique)
```


### GO BP overrepresentation for unique PD genes (WK2 vs WK1)
...
enrichGOResult_3 <- enrichGO(gene = PD_unique, OrgDb = org.Hs.eg.db, keyType = 'ENSEMBL', ont = "BP", pAdjustMethod = "BH", universe=all_genes, readable = T)
...
scorePlot <- RScorePlot_v2(enrichGOResult_3, resOrdered_WK2PD_WK1PD_LFCshrink_ens, showCategory = 30, textSize = 9, title = "" ) + theme(title = element_text(size=8), axis.text = element_text(size=8)) + ggtitle("GO BP terms for unique WK2:PD vs WK1:PD")
plot(scorePlot)
#data <- scorePlot$data
#write.table(data, file="Tofaris_Jan_June_21_PD_treated_WK2_vs_WK1_PD_unique_GO_BP_top.txt", sep="\t",quote=F)
```

### Reactome Pathways for unique PD genes (WK2 vs WK1)
...
library(ReactomePA)
# only takes entrez id
PD_unique_entrez <- resOrdered_WK2PD_WK1PD_LFCshrink_ens_sig %>% filter (rownames(.) %in% PD_unique) %>% dplyr::select("entrez") %>% drop_na() 
PD_unique_entrez <- as.character(PD_unique_entrez$entrez)

#geneChangeList <- sort(geneChangeList, decreasing=TRUE)
#desiredGeneChangeList <- names(geneChangeList)[abs(geneChangeList) >1] 
 all_genes <- as.character(unique(resOrdered_WK2PD_WK1PD_LFCshrink_ens$entrez))
enrich_Reactome_3 <-  enrichPathway(gene = PD_unique_entrez, pAdjustMethod = "BH", universe=all_genes, readable = T, organism = "human")
scorePlot <- RScorePlot_v2(enrich_Reactome_3, resOrdered_WK2PD_WK1PD_LFCshrink_ens, showCategory = 30, textSize = 9, title = "" ) + theme(title = element_text(size=8)) +ggtitle("Reactome terms for unique WK2:PD vs WK1:PD")
plot(scorePlot)
#data <- scorePlot$data
#write.table(data, file="Tofaris_Jan_June_21_PD_treated_WK2_vs_WK1_PD_unique_reactome_top.txt", sep="\t",quote=F)
```
...
#desiredGeneChangeList <- names(geneChangeList)[abs(geneChangeList) >1] 
 ...
 enrich_msig_c3 <-  enricher(gene = PD_unique_entrez, pAdjustMethod = "BH", universe=all_genes, TERM2GENE = m_t2g)



scorePlot <- RScorePlot_v4(enrich_msig_c3, resOrdered_WK2PD_WK1PD_LFCshrink_ens, showCategory = 30, textSize = 9, title = "" ) + theme(title = element_text(size=8), axis.text.y = element_text(size=9)) +ggtitle("MSigDB C3 Trascription factor genesets for unique WK2:PD vs WK1:PD")
plot(scorePlot)
```
...
transfac_pd_unique <- gost(query = PD_unique, organism = "hsapiens", domain_scope = "custom", custom_bg = all_genes, sources = "TF" , significant = T, correction_method = "g_SCS")
```
...
```{r}
NSC_UP_unique <- setdiff(sig_NSC_up,sig_PD_up)
NSC_DN_unique <- setdiff(sig_NSC_down, sig_PD_down)
...
scorePlot <- RScorePlot_v2(enrichGOResult_4, resOrdered_WK2NSC_WK1NSC_LFCshrink_ens, showCategory = 30, textSize = 9, title = "" ) + theme(title = element_text(size=8), axis.text = element_text(size=8)) + ggtitle("GO BP terms for unique WK2:NSC vs WK1:NSC")
plot(scorePlot)
#data <- scorePlot$data
#write.table(data, file="Tofaris_Jan_June_21_PD_treated_WK2_vs_WK1_NSC_unique_GO_BP_top.txt", sep="\t",quote=F)
```
...
scorePlot <- RScorePlot_v2(enrich_Reactome_4, resOrdered_WK2NSC_WK1NSC_LFCshrink_ens, showCategory = 30, textSize = 9, title = "" ) + theme(title = element_text(size=8), axis.text.y = element_text(size=8)) +ggtitle("Reactome terms for unique WK2:NSC vs WK1:NSC")
plot(scorePlot)
#data <- scorePlot$data
#write.table(data, file="Tofaris_Jan_June_21_PD_treated_WK2_vs_WK1_NSC_unique_reactome_top.txt", sep="\t",quote=F)
```
...
scorePlot <- RScorePlot_v4(enrich_msig_c3_2, resOrdered_WK2NSC_WK1NSC_LFCshrink_ens, showCategory = 30, textSize = 9, title = "" ) + theme(title = element_text(size=8), axis.text.y = element_text(size=9)) +ggtitle("MSigDB C3 Trascription factor genesets for unique WK2:NSC vs WK1:NSC")
plot(scorePlot)
```
...
all_genes <- as.character(unique(rownames(resOrdered_WK2NSC_WK1NSC_LFCshrink_ens)))
transfac_nsc_unique <- gost(query = NSC_unique, organism = "hsapiens", domain_scope = "custom", custom_bg = all_genes, sources = "TF" , significant = T, user_threshold = 0.05)
```
...
+ The common up and down genes between PD and NSC (WK2 vs WK1) probabaly signify time or age related biological change in the iPSC samples 
+ This is probably not related to the treatment

```{r}
overlapping_UP <- intersect(sig_NSC_up, sig_PD_up)
overlapping_DN <- intersect(sig_NSC_down, sig_PD_down)
...
library(org.Hs.eg.db)
 ...
 all_genes <- as.character(intersect(rownames(resOrdered_WK2NSC_WK1NSC_LFCshrink_ens), rownames(resOrdered_WK2PD_WK1PD_LFCshrink_ens)))
...
scorePlot <- RScorePlot_v2(enrichGOResult_5, resOrdered_WK2NSC_WK1NSC_LFCshrink_ens, showCategory = 30, textSize = 9, title = "" ) + theme(title = element_text(size=8), axis.text = element_text(size=8)) + ggtitle("GO BP terms for Common terms with NSC Rscore")
plot(scorePlot)

```


+ plot with Rscores based on the PD (WK1 vs WK2 DEG)

+ I dd check the NSC based Rscores plot, and thy were almost identical

```{r, eval=T}

scorePlot <- RScorePlot_v2(enrichGOResult_5, resOrdered_WK2PD_WK1PD_LFCshrink_ens, showCategory = 30, textSize = 9, title = "" ) + theme(title = element_text(size=8), axis.text = element_text(size=8)) + ggtitle("GO BP terms for Common terms with PD Rscore")
plot(scorePlot)
#data <- scorePlot$data
#write.table(data, file="Tofaris_Jan_June_21_PD_treated_WK2_vs_WK1_NSC_PD_common_GOBP_top.txt", sep="\t",quote=F)
```
...
#geneChangeList <- sort(geneChangeList, decreasing=TRUE)
#desiredGeneChangeList <- names(geneChangeList)[abs(geneChangeList) >1] 
 all_genes <- as.character(intersect(resOrdered_WK2NSC_WK1NSC_LFCshrink_ens$entrez, resOrdered_WK2PD_WK1PD_LFCshrink_ens$entrez))
enrich_Reactome_5 <-  enrichPathway(gene = overlapping_entrez, pAdjustMethod = "BH", universe=all_genes, readable = T, organism = "human")

scorePlot <- RScorePlot_v2(enrich_Reactome_5, resOrdered_WK2PD_WK1PD_LFCshrink_ens, showCategory = 30, textSize = 9, title = "" ) + theme(title = element_text(size=8), axis.text.y = element_text(size=8)) +ggtitle("Reactome terms for common genes with PD Rscore")
plot(scorePlot)
...
#data <- scorePlot$data
#write.table(data, file="Tofaris_Jan_June_21_PD_treated_WK2_vs_WK1_NSC_PD_common_reactome_top.txt", sep="\t",quote=F)
...
scorePlot <- RScorePlot_v4(enrich_msig_c3_3, resOrdered_WK2PD_WK1PD_LFCshrink_ens, showCategory = 30, textSize = 9, title = "" ) + theme(title = element_text(size=8), axis.text.y = element_text(size=9)) +ggtitle("MSigDB C3 Trascription factor genesets for common genes")
plot(scorePlot)
```
...
```{r}
gene_list_s <- list("PD_UP" = PD_UP_unique, "PD_DN"=PD_DN_unique, "NSC_UP"= NSC_UP_unique, "NSC_DN"= NSC_DN_unique, "overlap_UP"= overlapping_UP,
                    "overlap_DN" = overlapping_DN)

PD_UP_unique_entrez <- resOrdered_WK2PD_WK1PD_LFCshrink_ens_sig %>% filter (rownames(.) %in% PD_UP_unique) %>% dplyr::select("entrez") %>% drop_na() 
PD_UP_unique_entrez <- as.character(PD_UP_unique_entrez$entrez)

PD_DN_unique_entrez <- resOrdered_WK2PD_WK1PD_LFCshrink_ens_sig %>% filter (rownames(.) %in% PD_DN_unique) %>% dplyr::select("entrez") %>% drop_na()
PD_DN_unique_entrez <- as.character(PD_DN_unique_entrez$entrez)
...
gene_list_entrez <- list("PD_UP" = PD_UP_unique_entrez, "PD_DN"= PD_DN_unique_entrez, "NSC_UP"= NSC_UP_unique_entrez, 
...
p <- dotplot(GO_BP_cc_s,showCategory =10,by= "Percentage", group=F, font.size=8, size="count") 
 p <- p + scale_color_viridis_c(guide=guide_colorbar(reverse=TRUE)) + ggtitle("Enriched GO BP terms:WK1 vs WK2") + scale_size_continuous(breaks = c(10,30,50,75,100,150)) + geom_point(shape=21,stroke=0.5) + theme(title = element_text(size=8))
p
```
...
p <- dotplot(Reactome_cc,showCategory =10,by= "Percentage", group=F, font.size=8, size="count") 
 p <- p + scale_color_viridis_c(guide=guide_colorbar(reverse=TRUE)) + ggtitle("Enriched Reactome terms:WK1 vs WK2") + scale_size_continuous(breaks = c(10,30,50,75,100,150)) + geom_point(shape=21,stroke=0.5) + theme(title = element_text(size=8))
p
```
...
p <- dotplot(KEGG_cc,showCategory =10,by= "Percentage", group=F, font.size=8, size="count") 
 p <- p + scale_color_viridis_c(guide=guide_colorbar(reverse=TRUE)) + ggtitle("Enriched KEGG terms:WK1 vs WK2") + scale_size_continuous(breaks = c(10,30,50,75,100,150)) + geom_point(shape=21,stroke=0.5) + theme(title = element_text(size=8))
p
```
...
all_genes <- rownames(resOrdered_WK2PD_WK1PD_LFCshrink_ens)
WK2_WK1_PD <- c(sig_PD_up, sig_PD_down)
```

```{r, eval=T}

WK2_WK1_PD_pdrisk_ppi <- test_enrich_two_sets_avi(input_net = ppi_net ,input_cds = cds_gene,input_list_1 = WK2_WK1_PD , input_list_2 =PD_risk_genes_ens  ,input_universe = all_genes,n_rand = 10000 )
```
...
```{r, eval=T}
all_genes <- rownames(resOrdered_WK2PD_WK1PD_LFCshrink_ens)
PD_unique_overlap <- test_enrich_two_sets_avi(input_net = ppi_net ,input_cds = cds_gene,input_list_1 = PD_unique , input_list_2 = PD_risk_genes_ens ,input_universe = all_genes,n_rand = 10000 )
```
...
WK2_WK1_PD_async_ppi <- test_enrich_two_sets_avi(input_net = ppi_net ,input_cds = cds_gene,input_list_1 = WK2_WK1_PD , input_list_2 = async_int_proteins_ens_v  ,input_universe = all_genes,n_rand = 10000 )
```
...
```{r, eval=T}
all_genes <- rownames(resOrdered_WK2PD_WK1PD_LFCshrink_ens)
PD_unique_overlap_async <- test_enrich_two_sets_avi(input_net = ppi_net ,input_cds = cds_gene,input_list_1 = PD_unique , input_list_2 = async_int_proteins_ens_v ,input_universe = all_genes,n_rand = 10000 )
```

...
# Differential Expression : Aggregate vs NSC (WK1)
...
my_plot_counts <- function(obj, plot_genes=NULL, intgroup_vec=c("Group_2", "Replicate_2", "Batch","Treatment"), t2g_info=NULL, n_ncol=4, n_nrow =6, title=NULL){
  ...
  # plot_data_all[, gene_print := paste0(gene_idx, "  ", gene)]
  plot_data_all[, gene_print := paste0(gene_idx, "  ", ext_gene)] ## use only gene symbol as facet title
  plot_data_all <- plot_data_all %>% filter( Group_2 %in% c("WK1_NSC","WK1_Aggregate"))
...
control_genes_sig <- resOrdered_WK1_Agg_NSC_LFCshrink %>% filter(rownames(.) %in% control_genes & padj<0.05 )
...
```{r, fig.width=8}
pdgwas_genes_sig <- resOrdered_WK1_Agg_NSC_LFCshrink %>% filter(rownames(.) %in% PD_risk_genes & padj<0.05 & log2FoldChange < 0 )
my_plot_counts(dds, plot_genes = rownames(pdgwas_genes_sig),title=" GWAS PD risk genes at 5% FDR", t2g_info = t2g_genes, n_ncol = 5)
```
...
• Very little ovelap between WK1 Agg vs NSC DEG and PD GWAS risk genes

• 12 a-synuclein interacting proteins based on mass spet overlap with downregulated genes
...
```{r, echo=FALSE, eval=T}
  ...
  d <- "Overlap of WK1 Agg vs NSC DEG with PD related genes"
res_print_WK1_Agg_NSC <- as.data.frame(res_print_WK1_Agg_NSC)
rownames(res_print_WK1_Agg_NSC) <- res_print_WK1_Agg_NSC$ens_gene



  ## Venn Diagram: DEGs for all 3 pairwise comparisons
  sig_WK1_up <- unique(rownames(res_print_WK1_Agg_NSC[res_print_WK1_Agg_NSC$fc == "up", ]))
  sig_WK1_down <- unique(rownames(res_print_WK1_Agg_NSC[res_print_WK1_Agg_NSC$fc == "down", ]))
 ...
 # cairo_ps("./DEG_genes_Venn_diagram.eps", height = 4, width = 5, onefile = F)
   venn.plot <- venn.diagram(x=list(S1=as.matrix(sig_WK1_up), S2=as.matrix(sig_WK1_down), S3=as.matrix(PD_risk_genes_ens), S4=as.matrix(async_int_proteins_ens_v)), fill = c("red", "cyan3", "green3","yellow"), alpha=c(0.5, 0.5, 0.5,0.5), cex=0.6, cat.cex=0.55, fontfamily="sans", cat.fontfamily="sans", main.fontfamily = "sans", cat.dist=c(0.2,0.2,0.1,0.1),  filename=NULL, euler.d=F, scaled=F, print.mode=c("percent", "raw"), sigdigs = 2, category.names=paste0(c("WK1 UP", "WK1 Down","PD GWAS genes","a-synuclein interacting proteins"), "\n", c(length(sig_WK1_up), length(sig_WK1_down), length(PD_risk_genes_ens), length(async_int_proteins_ens_v))), main = d, main.cex = 0.7)
...
• Test signfiicance of overlap between the various sets by using fisehrs exact

• heatmap showing Odds ratio as colour gradient and pvalue signficance as text

• No significant overlap , but OR > 1.0 means strong overlap between two catgeories

```{r, warning=FALSE}
deg_list <- list("WK1_Agg_vs_NSC_UP" = sig_WK1_up , "WK1_Agg_vs_NSC_DN" = sig_WK1_down)
pd_gene_l <- list("PD_GWAS_genes" = PD_risk_genes_ens, "a-sync_int_proteins" = async_int_proteins_ens_v)
gs.RNAseq =  as.numeric(length(rownames(resOrdered_WK1_Agg_NSC_LFCshrink_ens)))
...
```{r}
cat("the genes which are downregulated in WK1 Agg vs NSC & also found to be a-synculein interacting proteins" , fill=T)
p <- intersect(sig_WK1_down,async_int_proteins_ens_v)
p_df <- async_int_proteins_ens %>% filter(ens_gene %in% p)
print(p_df$gene)
...
#desiredGeneChangeList <- names(geneChangeList)[abs(geneChangeList) >1] 
 ...
 m_hpo <- msigdbr::msigdbr(species = "Homo sapiens", category = "C5", subcategory = "HPO")%>% dplyr::select(gs_name, entrez_gene)
 
 
 
 enrich_msig_hpo_1 <-  enricher(gene = geneChangeList, pAdjustMethod = "BH", universe=all_genes, TERM2GENE = m_hpo)



scorePlot <- RScorePlot_v4(enrich_msig_hpo_1, resOrdered_WK1_Agg_NSC_LFCshrink_ens, showCategory = 15, textSize = 9, title = "" ) + theme(title = element_text(size=8), axis.text.y = element_text(size=9)) +ggtitle("HPO genesets for WK1:Aggregate vs NSC")
plot(scorePlot)
```
...
### MSigDB C3: Traancription factor target genesets for PD genes
...
#desiredGeneChangeList <- names(geneChangeList)[abs(geneChangeList) >1] 
 ...
 enrich_msig_c3_1 <-  enricher(gene = geneChangeList, pAdjustMethod = "BH", universe=all_genes, TERM2GENE = m_t2g)



scorePlot <- RScorePlot_v3(enrich_msig_c3_1, resOrdered_WK1_Agg_NSC_LFCshrink_ens, showCategory = 10, textSize = 9, title = "" ) + theme(title = element_text(size=8), axis.text.y = element_text(size=9)) +ggtitle(" Trascription factor genesets for WK1:Aggregate vs NSC")
plot(scorePlot)
```
...
# Differential Expression : : Aggregate vs NSC (WK2)
...
my_plot_counts <- function(obj, plot_genes=NULL, intgroup_vec=c("Group_2", "Replicate_2", "Batch","Treatment"), t2g_info=NULL, n_ncol=4, n_nrow =6, title=NULL){
  ...
  # plot_data_all[, gene_print := paste0(gene_idx, "  ", gene)]
  plot_data_all[, gene_print := paste0(gene_idx, "  ", ext_gene)] ## use only gene symbol as facet title
  plot_data_all <- plot_data_all %>% filter( Group_2 %in% c("WK2_NSC","WK2_Aggregate"))
  p <- ggplot(data = plot_data_all, aes(x=Group_2, y=log2(count+1), colour=Group_2, shape=Treatment))
  p <- p + geom_violin(aes(group=Group_2), alpha = 0, colour = "gray60", scale="width")
  ## p <- p + ggbeeswarm::geom_quasirandom(alpha = 0.6, size = 3, groupOnX = TRUE)
  p <- p + geom_point(position = position_jitterdodge(), size=5, alpha=0.6)
...
### PD GWAS genes (NALLS 2019 & Chang 2017) UP

•overlap of PD risk genes with DEG

```{r, fig.width=8, fig.height=4}
pdgwas_genes_sig <- resOrdered_WK2_Agg_NSC_LFCshrink %>% filter(rownames(.) %in% PD_risk_genes & padj<0.05 & log2FoldChange > 0 )
my_plot_counts(dds, plot_genes = rownames(pdgwas_genes_sig),title=" GWAS PD genes at 5% FDR", t2g_info = t2g_genes)
```
...
• Very little ovelap between WK2 Agg vs NSC DEG and PD GWAS risk genes

• 12  a-synuclein interacting proteins based on mass spec overlap with upregulated genes of which 1 is also a PD GWAS gene

• 8 a-synuclein interacting proteins based on mass spec overlap with downregulated genes 
...
```{r, echo=FALSE, eval=T}
  ...
  d <- "Overlap of Agg vs NSC (WK2) with PD related genes"
res_print_WK2_Agg_NSC <- as.data.frame(res_print_WK2_Agg_NSC)
rownames(res_print_WK2_Agg_NSC) <- res_print_WK2_Agg_NSC$ens_gene



  ## Venn Diagram: DEGs for all 3 pairwise comparisons
  sig_WK2_up <- unique(rownames(res_print_WK2_Agg_NSC[res_print_WK2_Agg_NSC$fc == "up", ]))
  sig_WK2_down <- unique(rownames(res_print_WK2_Agg_NSC[res_print_WK2_Agg_NSC$fc == "down", ]))
  
 
 # tiff(filename = "./DEG_genes_WK1_WK2_aggregate_vs_NSC_Venn_diagram.tiff", height = 4, width=6, units = "in", res=300)
 
 # cairo_ps("./DEG_genes_Venn_diagram.eps", height = 4, width = 5, onefile = F)
   venn.plot <- venn.diagram(x=list(S1=as.matrix(sig_WK2_up), S2=as.matrix(sig_WK2_down), S3=as.matrix(PD_risk_genes_ens), S4=as.matrix(async_int_proteins_ens_v)), fill = c("red", "cyan3", "green3","yellow"), alpha=c(0.5, 0.5, 0.5,0.5), cex=0.6, cat.cex=0.55, fontfamily="sans", cat.fontfamily="sans", main.fontfamily = "sans", cat.dist=c(0.2,0.2,0.1,0.1),  filename=NULL, euler.d=F, scaled=F, print.mode=c("percent", "raw"), sigdigs = 2, category.names=paste0(c("WK2 UP", "WK2 Down","PD GWAS genes","a-synuclein interacting proteins"), "\n", c(length(sig_WK2_up), length(sig_WK2_down), length(PD_risk_genes_ens), length(async_int_proteins_ens_v))), main = d, main.cex = 0.7)
...
• heatmap showing Pdds Ratio as colours and Pvalue signfiicance as * or NS

• OR > 1.0 means there is a strong association between the categories


```{r, warning=FALSE}
deg_list <- list("WK2_Agg_vs_NSC_UP" = sig_WK2_up , "WK2_Agg_vs_NSC_DN" = sig_WK2_down)
pd_gene_l <- list("PD_GWAS_genes" = PD_risk_genes_ens, "a-sync_int_proteins" = async_int_proteins_ens_v)
gs.RNAseq =  as.numeric(length(rownames(resOrdered_WK2_Agg_NSC_LFCshrink_ens)))
...
```{r}
cat("the genes which are UP in WK2 Agg vs NSC & also found to be a-synculein interacting proteins" , fill=T)
#sig_WK2 <- c(sig_WK2_up, sig_WK2_down)
p <- intersect(sig_WK2_up,async_int_proteins_ens_v)
p_df <- async_int_proteins_ens %>% filter(ens_gene %in% p)
print(p_df$gene) 


cat("the genes which are Down in WK2 Agg vs NSC & also found to be a-synculein interacting proteins" , fill=T)
#sig_WK2 <- c(sig_WK2_up, sig_WK2_down)
p <- intersect(sig_WK2_down,async_int_proteins_ens_v)
p_df <- async_int_proteins_ens %>% filter(ens_gene %in% p)
print(p_df$gene)
...
```
```{r, eval=F}
reactome_wk2_agg_nsc <- scorePlot$data
write.table(reactome_wk2_agg_nsc, file="Jan_Jun_21_ALL_pooled_WK2_Agg_vs_NSC_DEG_top_reactome_terms.txt", sep="\t",quote=F)
```
...
#desiredGeneChangeList <- names(geneChangeList)[abs(geneChangeList) >1] 
 ...
 enrich_msig_hpo_2 <-  enricher(gene = geneChangeList, pAdjustMethod = "BH", universe=all_genes, TERM2GENE = m_hpo)



scorePlot <- RScorePlot_v3(enrich_msig_hpo_2, resOrdered_WK2_Agg_NSC_LFCshrink_ens, showCategory = 10, textSize = 9, title = "" ) + theme(title = element_text(size=8), axis.text.y = element_text(size=9)) +ggtitle("HPO genesets for WK2:Aggregate vs NSC")
plot(scorePlot)
```
...
#desiredGeneChangeList <- names(geneChangeList)[abs(geneChangeList) >1] 
 ...
 enrich_msig_c3_2 <-  enricher(gene = geneChangeList, pAdjustMethod = "BH", universe=all_genes, TERM2GENE = m_t2g)



scorePlot <- RScorePlot_v4(enrich_msig_c3_2, resOrdered_WK2_Agg_NSC_LFCshrink_ens, showCategory = 30, textSize = 9, title = "" ) + theme(title = element_text(size=8), axis.text.y = element_text(size=9)) +ggtitle(" Trascription factor genesets for WK2:Aggregate vs NSC")
plot(scorePlot)
```
...
# Overlap of DEG: AGG vs NSC DEG (WK 1 vs WK2)

• To understand the trends of DEG between AGG vs NSC at WK1 and WK2
• Is there an overlap of upregulated or downregulated genes, or is the trend opposite, 
or are things going up after treatment in WK1 going down WK2 or vice versa
...
## Plot counts for common genes 
+ visualise the counts for the genes which are commonly upregulated and downreugulated in both WK1 and WK2 for AGG vs NSC comparison


```{r}
common_up <- intersect(sig_WK1_up, sig_WK2_up)
common_up_v1 <- t2g_genes %>% filter(ens_gene %in% common_up) %>% select(gene) 

common_down <- intersect(sig_WK1_down, sig_WK2_down)
common_down_v1 <- t2g_genes %>% filter(ens_gene %in% common_down) %>% select(gene) 
...
```{r, echo=F}
my_plot_counts <- function(obj, plot_genes=NULL, intgroup_vec=c("Group_2", "Replicate_2", "Batch","Treatment","Treatment_time","Stimulation"), t2g_info=NULL, n_ncol=4, n_nrow =6, title=NULL){
  ...
  #plot_data_all <- plot_data_all %>% filter(Group_2 %in% c("WK2_Aggregate" ,"WK1_Aggregate"))
  p <- ggplot(data = plot_data_all, aes(x=Group_2, y=log2(count+1), colour=Stimulation, shape=Treatment, group=Treatment_time))
  p <- p + geom_violin(aes(group=Group_2), alpha = 0, colour = "gray60", scale="width")
  ## p <- p + ggbeeswarm::geom_quasirandom(alpha = 0.6, size = 3, groupOnX = TRUE)
  p <- p + geom_point(position = position_jitterdodge(), size=5, alpha=0.6) 
  p <- p + xlab(NULL)
...
p <-my_plot_counts(dds, plot_genes = common_up_v1$gene,title=" Common UP genes in Aggregate vs NSC (WK1 vs WK2)", t2g_info = t2g_genes, n_ncol = 2)
p
```

```{r, echo=FALSE, fig.height=10, fig.width=12}
p <-my_plot_counts(dds, plot_genes = common_down_v1$gene,title=" Common UP genes in Aggregate vs NSC (WK1 vs WK2)", t2g_info = t2g_genes, n_ncol = 6)
p
```



# Differential Expression : Week2 Aggregate vs Week1 Aggregate

 * design " ~ Batch + Treatment_time + Stimulation + TreatmentTime:Stimulation
* DEG log2FC threshold = 0.50 and FDR 5%
* Accounting for differences in WK2 vs WK1 Aggregates
+ contrasts of interest: Treatment_time_WK2_vs_WK1 + Treatment_timeWK2.StimulationAggregate
...
res_WK2Agg_WK1Agg <- results(object=dds ,contrast = list(c("Treatment_time_WK2_vs_WK1","Treatment_timeWK2.StimulationAggregate")) , lfcThreshold = 0.50, alpha=0.05)
...
```{r}
res_WK2Agg_WK1Agg_LFCshrink <- lfcShrink(dds = dds ,contrast = list(c("Treatment_time_WK2_vs_WK1","Treatment_timeWK2.StimulationAggregate")) ,type="ashr")
...
res_print_WK2Agg_WK1Agg <- print_DESeq_table(res_WK2Agg_WK1Agg_LFCshrink, t2g_info = t2g_genes, alpha_level = alpha_cutoff,logfcthreshold = 0.5)
...
```{r, echo=F}
my_plot_counts <- function(obj, plot_genes=NULL, intgroup_vec=c("Group_2", "Batch","Treatment"), t2g_info=NULL, n_ncol=4, n_nrow =6, title=NULL){
...
```{r, echo=FALSE, fig.height=9, fig.width=10}
my_plot_counts(dds, plot_genes = plot_up_genes,title="upregulated genes in WK2 Aggregate vs WK1 Aggregate", t2g_info = t2g_genes, n_ncol = 5)
```
...
```{r, echo=FALSE, fig.height=9, fig.width=10}
my_plot_counts(dds, plot_genes = plot_down_genes,title="downregulated genes in WK2 Aggregate vs WK1 Aggregate", t2g_info = t2g_genes, n_ncol = 5)
```
...
my_plot_counts(dds, plot_genes = rownames(pdgwas_genes_sig),title=" GWAS PD genes at 5% FDR and log2FC >= 0.5", t2g_info = t2g_genes, n_ncol=4)
```
...
```{r, fig.height =8, fig.widht=9}
pdgwas_genes_sig <- resOrdered_WK2Agg_WK1Agg_LFCshrink %>% filter(rownames(.) %in% PD_risk_genes & padj<0.05 & log2FoldChange <= -0.5 )
my_plot_counts(dds, plot_genes = rownames(pdgwas_genes_sig),title=" GWAS PD genes at 5% FDR & log2FC <= -0.5", t2g_info = t2g_genes, n_ncol=4)
```
...
write.table(resOrdered_WK2Agg_WK1Agg_LFCshrink_ens, file="deseq2_results_Jan_Jun_21_WK2_vsWK1_Aggregated.txt", sep="\t", quote=F)
...
• 58  a-synuclein interacting proteins based on mass spec overlap with upregulated genes of 

• 58 a-synuclein interacting proteins based on mass spec overlap with downregulated genes 
...
• heatmap showing  Odds ratio as colour gradient and pvalue significance att FDR 5% as * or NS

• OR > 1.0 means significant association betweent the categories

• Overlap between alpha-synuclein interacting proteins and Upregulated proteins in WK2 Agg vs WK1 Agg is significant
...
```{r}
cat("the genes which are UP in WK2 Agg vs WK1 Agg & also found to be a-synculein interacting proteins" , fill=T)
#sig_WK2 <- c(sig_WK2_up, sig_WK2_down)
p <- intersect(sig_Agg_up,async_int_proteins_ens_v)
p_df <- async_int_proteins_ens %>% filter(ens_gene %in% p)
print(p_df$gene) 


cat("the genes which are Down in WK2 Agg vs WK1 Agg & also found to be a-synculein interacting proteins" , fill=T)
#sig_WK2 <- c(sig_WK2_up, sig_WK2_down)
p <- intersect(sig_Agg_down,async_int_proteins_ens_v)
p_df <- async_int_proteins_ens %>% filter(ens_gene %in% p)
print(p_df$gene)
...
library(EnhancedVolcano)
 Vol <- EnhancedVolcano(resOrdered_WK2Agg_WK1Agg_LFCshrink_ens,
    ...
    x = 'log2FoldChange',
    y = 'pvalue', subtitle = "", title = "WK2:Agg vs WK1:Agg", xlim = c(-26,26), labSize =3, FCcutoff = 1.5, axisLabSize = 6, titleLabSize = 8, legendLabSize = 6, pCutoff = 0.05, pCutoffCol = "padj",   drawConnectors = F, captionLabSize = 6,
...
resOrdered_WK2Agg_WK1Agg_LFCshrink_ens_sig <- resOrdered_WK2Agg_WK1Agg_LFCshrink_ens %>% filter(padj < 0.05 & abs(log2FoldChange) >= 0.50) 
...
enrichGOResult_3 <- enrichGO(gene = geneChangeList, OrgDb = org.Hs.eg.db, keyType = 'ENSEMBL', ont = "BP", pAdjustMethod = "BH", universe=all_genes, readable = T)
...
scorePlot <- RScorePlot_v2(enrichGOResult_3, resOrdered_WK2Agg_WK1Agg_LFCshrink_ens, showCategory = 20, textSize = 9, title = "" ) + theme(title = element_text(size=8))+ggtitle("GO BP terms for WK2:Agg vs WK1:Agg")
plot(scorePlot)

#WK2_WK1_Agg_deg_top_goterms <- scorePlot$data
#write.table(WK2_WK1_Agg_deg_top_goterms, file="Jan_Jun_21_ALL_pooled_WK2_vs_WK1_Aggregate_DEG_top_GOBP.txt", sep="\t",quote=F)
```
...
enrich_Reactome_3 <-  enrichPathway(gene = geneChangeList, pAdjustMethod = "BH", universe=all_genes, readable = T, organism = "human")
scorePlot <- RScorePlot_v2(enrich_Reactome_3, resOrdered_WK2Agg_WK1Agg_LFCshrink_ens, showCategory = 25, textSize = 9, title = "" ) + theme(title = element_text(size=8))+ ggtitle("Reactome terms for WK2:Agg vs WK1:Agg")
plot(scorePlot)
#WK2_WK1_Agg_deg_top_reactome <- scorePlot$data
#write.table(WK2_WK1_Agg_deg_top_reactome, file="Jan_Jun_21_ALL_pooled_WK2_vs_WK1_Aggregate_DEG_top_reactome.txt", sep="\t",quote=F)
...
#desiredGeneChangeList <- names(geneChangeList)[abs(geneChangeList) >1] 
 ...
 enrich_msig_hpo_3 <-  enricher(gene = geneChangeList, pAdjustMethod = "BH", universe=all_genes, TERM2GENE = m_hpo)



scorePlot <- RScorePlot_v4(enrich_msig_hpo_3,  resOrdered_WK2Agg_WK1Agg_LFCshrink_ens, showCategory = 10, textSize = 9, title = "" ) + theme(title = element_text(size=8), axis.text.y = element_text(size=9)) +ggtitle("HPO genesets for WK2:Agg vs WK1:Agg")
plot(scorePlot)
```
...
#desiredGeneChangeList <- names(geneChangeList)[abs(geneChangeList) >1] 
 ...
 enrich_msig_c3_3 <-  enricher(gene = geneChangeList, pAdjustMethod = "BH", universe=all_genes, TERM2GENE = m_t2g)



scorePlot <- RScorePlot_v4(enrich_msig_c3_3, resOrdered_WK2Agg_WK1Agg_LFCshrink_ens, showCategory = 10, textSize = 9, title = "" ) + theme(title = element_text(size=8), axis.text.y = element_text(size=9)) +ggtitle(" Trascription factor genesets for WK2:Agg vs WK1:Agg")
plot(scorePlot)
```
...
```{r,echo=FALSE, fig.height=4, fig.width=9, eval=T}
control_genes <- c("OPA1 - ENSG00000198836","MFN1 - ENSG00000171109","TOMM20 - ENSG00000173726","DNM1L - ENSG00000087470","TFAM - ENSG00000108064","NRF1 - ENSG00000106459","NFE2L2 - ENSG00000116044","MTOR - ENSG00000198793","HIF1A - ENSG00000100644","ARNT - ENSG00000143437","SNCA - ENSG00000145335","PARK7 - ENSG00000116288","OMA1 - ENSG00000162600","MFN1 - ENSG00000171109")
...
my_plot_counts(dds, plot_genes = rownames(pdgwas_genes_sig),title=" GWAS PD genes at 5% FDR and log2FC >= 0.5", t2g_info = t2g_genes, n_ncol=5)
```
...
```{r, fig.height=8, fig.width=12}
pdgwas_genes_sig <- resOrdered_WK2NSC_WK1NSC_LFCshrink %>% filter(rownames(.) %in% PD_risk_genes & padj<0.05 & log2FoldChange <= -0.5)
my_plot_counts(dds, plot_genes = rownames(pdgwas_genes_sig),title=" GWAS PD genes at 5% FDR and Log@FC <= -0.5", t2g_info = t2g_genes, n_ncol=5)
```
...
write.table(resOrdered_WK2NSC_WK1NSC_LFCshrink_ens, file="deseq2_results_Jan_Jun_21_WK2_NSC_vsWK1_NSC_all.txt", sep="\t", quote=F)
...
• 157  a-synuclein interacting proteins based on mass spec overlap with upregulated genes of which 2 are also a PD GWAS gene

• 133 a-synuclein interacting proteins based on mass spec overlap with downregulated genes 
...
• heatmap showing Odds Ratio as colours and pvalue significance at FDR 5% as * or NS 

• OR > 1 = sttrong association between categories

• No statistically significant overlap between DEG and PD GWAS genes and a-synulein interacting proteins
...
```{r}
cat("the genes which are UP in WK2 NSC vs WK1 NSC & also found to be a-synculein interacting proteins" , fill=T)
#sig_WK2 <- c(sig_WK2_up, sig_WK2_down)
p <- intersect(sig_NSC_up,async_int_proteins_ens_v)
p_df <- async_int_proteins_ens %>% filter(ens_gene %in% p)
print(p_df$gene) 


cat("the genes which are Down in WK2 NSC vs WK1 NSC & also found to be a-synculein interacting proteins" , fill=T)
#sig_WK2 <- c(sig_WK2_up, sig_WK2_down)
p <- intersect(sig_NSC_down,async_int_proteins_ens_v)
p_df <- async_int_proteins_ens %>% filter(ens_gene %in% p)
print(p_df$gene)
...
enrichGOResult_4 <- enrichGO(gene = geneChangeList, OrgDb = org.Hs.eg.db, keyType = 'ENSEMBL', ont = "BP", pAdjustMethod = "BH", universe=all_genes, readable = T)
...
scorePlot <- RScorePlot_v2(enrichGOResult_4, resOrdered_WK2NSC_WK1NSC_LFCshrink_ens, showCategory = 25, textSize = 9, title = "" ) + theme(title = element_text(size=8))+ggtitle("GO BP terms for WK2:NSC vs WK1:NSC")
plot(scorePlot)
```
...
enrich_Reactome_4 <-  enrichPathway(gene = geneChangeList, pAdjustMethod = "BH", universe=all_genes, readable = T, organism = "human")
scorePlot <- RScorePlot_v2(enrich_Reactome_4, resOrdered_WK2NSC_WK1NSC_LFCshrink_ens, showCategory = 25, textSize = 9, title = "" ) + theme(title = element_text(size=8))+ ggtitle("Reactome terms for WK2:NSC vs WK1:NSC")
plot(scorePlot)
```
...
#desiredGeneChangeList <- names(geneChangeList)[abs(geneChangeList) >1] 
 ...
 enrich_msig_hpo_4 <-  enricher(gene = geneChangeList, pAdjustMethod = "BH", universe=all_genes, TERM2GENE = m_hpo)



scorePlot <- RScorePlot_v4(enrich_msig_hpo_4,  resOrdered_WK2NSC_WK1NSC_LFCshrink_ens, showCategory = 10, textSize = 9, title = "" ) + theme(title = element_text(size=8), axis.text.y = element_text(size=9)) +ggtitle("HPO genesets for WK2:NSC vs WK1:NSC")
plot(scorePlot)
```
...
#desiredGeneChangeList <- names(geneChangeList)[abs(geneChangeList) >1] 
 ...
 enrich_msig_c3_4 <-  enricher(gene = geneChangeList, pAdjustMethod = "BH", universe=all_genes, TERM2GENE = m_t2g)



scorePlot <- RScorePlot_v4(enrich_msig_c3_4, resOrdered_WK2NSC_WK1NSC_LFCshrink_ens, showCategory = 10, textSize = 9, title = "" ) + theme(title = element_text(size=8), axis.text.y = element_text(size=9)) +ggtitle(" Trascription factor genesets for WK2:NSC vs WK1:NSC")
plot(scorePlot)
```
...
# VennDiagram: AGG DEG (WK1 vs WK2) vs NSC DEG (WK 1 vs WK2) 

+ As we can see there is a 30% overlap in the genes which are changing (up or down) between week1 and week2 for NSC and Aggregated samples.
+ To get a Agg and NSC specific signal we , need to only look at the gene subsets that do not overlap.

```{r, echo=FALSE, eval=T, fig.width=6}
...
* contrast terms : "Treatment_timeWK2.StimulationAggregate"
* design " ~ Batch + Treatment_time + Stimulation + TreatmentTime:Stimulation
...
res_WK2WK1_AggNSC <- results(object=dds ,contrast = list("Treatment_timeWK2.StimulationAggregate") , lfcThreshold = 0.0, alpha=0.05)
 mcols(res_WK2WK1_AggNSC)$description
 res_summary <- capture.output(summary(res_WK2WK1_AggNSC))
 cat(res_summary, sep="  \n* ")
 
 # number of differentially expressed genes based on cut off
 
 num_DEG <- sum(res_WK2WK1_AggNSC$padj < alpha_cutoff & abs(res_WK2WK1_AggNSC$log2FoldChange) > 0, na.rm=TRUE)
    cat("  \n* Genes with adjusted P-value not NA (universe of genes): ", sum(!is.na(res_WK2WK1_AggNSC$padj)))
    cat("  \n* Differentially expressed genes with log2fc > 0 & p.adjusted <0.05: ", num_DEG, "  \n")
resOrdered_WK2WK1_AggNSC <- res_WK2WK1_AggNSC[order(res_WK2WK1_AggNSC$padj),]
resOrdered_WK2WK1_AggNSC <- as.data.frame(resOrdered_WK2WK1_AggNSC )
#write.table(as.data.frame(resOrdered_WK1_PD_NSC), file = paste0("deseq2_results_","WK1_PD_vs._NSC",".txt"), sep="\t", quote=F)
```
...
```{r}
res_WK2WK1_AggNSC_LFCshrink <- lfcShrink(dds = dds ,contrast = list("Treatment_timeWK2.StimulationAggregate") ,type="ashr")
DESeq2::plotMA(res_WK2WK1_AggNSC_LFCshrink, ylim = c(-20,20))
resOrdered_WK2WK1_AggNSC_LFCshrink <- res_WK2WK1_AggNSC_LFCshrink[order(res_WK2WK1_AggNSC_LFCshrink$padj),]
resOrdered_WK2WK1_AggNSC_LFCshrink  <- as.data.frame(resOrdered_WK2WK1_AggNSC_LFCshrink)
```
...
res_print_WK2WK1_AggNSC <- print_DESeq_table(res_WK2WK1_AggNSC_LFCshrink, t2g_info = t2g_genes, alpha_level = alpha_cutoff,logfcthreshold = 0)


    
    plot_up_genes <- res_print_WK2WK1_AggNSC[fc == "up", target_id][1:top_genes]
    plot_up_genes <- plot_up_genes[!(is.na(plot_up_genes))] # if there are not top_genes DEGs, NA values will be returned
    plot_down_genes <- res_print_WK2WK1_AggNSC[fc == "down", target_id][1:top_genes]
    plot_down_genes <- plot_down_genes[!(is.na(plot_down_genes))]
  
    
    
   res_print_WK2WK1_AggNSC <-res_print_WK2WK1_AggNSC[, gene_type := NULL] ## only protein coding genes
  res_print_WK2WK1_AggNSC <- res_print_WK2WK1_AggNSC[, target_id := NULL] ## remove "symbol - ensembl_ID"
    
    setnames(res_print_WK2WK1_AggNSC, "desc", "___________long_gene_description___________")
   res_print_WK2WK1_AggNSC <- res_print_WK2WK1_AggNSC %>% drop_na()
```



```{r}
res_print_WK2WK1_AggNSC %>% DT::datatable(caption="Interaction Treatment Time vs Condition", escape=F, extensions = 'Buttons', options = list(dom = 'Blfrtip', buttons = list('colvis', list(extend = 'copy', exportOptions = list(columns=':visible')), list(extend = 'csv', exportOptions = list(columns=':visible')), list(extend = 'excel', exportOptions = list(columns=':visible')))))
```
...
#t2g_genes_control <- t2g_genes %>% filter(gene %in% control_genes) %>% select(gene)
control_genes_sig <- resOrdered_WK2WK1_AggNSC_LFCshrink %>% filter(rownames(.) %in% control_genes & padj<0.1 )
my_plot_counts(dds, plot_genes = rownames(control_genes_sig),title="interesting significant PD genes", t2g_info = t2g_genes)
#PGC-1alpha = ENSG00000109819
# OMA1 - ENSG00000162600
# OPA1 - ENSG00000198836
...
### PD GWAS genes (Nalls 2019 & Chang 2017) 

• No signfiicant PD risk genes overlap with the interaction results

```{r, eval=FALSE}
pdgwas_genes_sig <- resOrdered_WK2WK1_AggNSC_LFCshrink %>% filter(rownames(.) %in% PD_risk_genes & padj< 0.05)
my_plot_counts(dds, plot_genes = rownames(pdgwas_genes_sig),title=" GWAS PD genes at 5% FDR", t2g_info = t2g_genes)
```
...
resOrdered_WK2WK1_AggNSC_LFCshrink$gene <- rownames(resOrdered_WK2WK1_AggNSC_LFCshrink)
resOrdered_WK2WK1_AggNSC_LFCshrink_ens <- merge(resOrdered_WK2WK1_AggNSC_LFCshrink, t2g_genes, by.all ="gene")
rownames(resOrdered_WK2WK1_AggNSC_LFCshrink_ens) <- resOrdered_WK2WK1_AggNSC_LFCshrink_ens$ens_gene
rm(resOrdered_WK2WK1_AggNSC)
resOrdered_WK2WK1_AggNSC_LFCshrink_ens <- resOrdered_WK2WK1_AggNSC_LFCshrink_ens[,c(1:7,11)]

# get rid of NAs for clusterprofiler
#library(tidyr)
resOrdered_WK2WK1_AggNSC_LFCshrink_ens <- resOrdered_WK2WK1_AggNSC_LFCshrink_ens%>% drop_na()

# map ids using annotation db
library(org.Hs.eg.db)
library(AnnotationDbi)

resOrdered_WK2WK1_AggNSC_LFCshrink_ens$entrez <- mapIds(org.Hs.eg.db, keys= rownames(resOrdered_WK2WK1_AggNSC_LFCshrink_ens), column = "ENTREZID", keytype = "ENSEMBL", multiVals = "first")
resOrdered_WK2WK1_AggNSC_LFCshrink_ens$symbol <- mapIds(org.Hs.eg.db, keys= rownames(resOrdered_WK2WK1_AggNSC_LFCshrink_ens), column = "SYMBOL", keytype = "ENSEMBL", multiVals = "first")
write.table(resOrdered_WK2WK1_AggNSC_LFCshrink_ens, file="deseq2_results_Jan_Jun_21_WK2WK1_vs_NSCAgg_interaction.txt", sep="\t", quote=F)
...
• 6  a-synuclein interacting proteins based on mass spec overlap with upregulated genes 

• 1 a-synuclein interacting proteins based on mass spec overlap with downregulated genes 
...
```{r, echo=FALSE, eval=T}
  ...
  d <- "Overlap of WK2 vs WK1:Agg vs NSC with PD related genes"
res_print_WK2WK1_AggNSC<- as.data.frame(res_print_WK2WK1_AggNSC)
rownames(res_print_WK2WK1_AggNSC) <- res_print_WK2WK1_AggNSC$ens_gene



  ## Venn Diagram: DEGs for all 3 pairwise comparisons
  sig_int_up <- unique(rownames(res_print_WK2WK1_AggNSC[res_print_WK2WK1_AggNSC$fc == "up", ]))
  sig_int_down <- unique(rownames(res_print_WK2WK1_AggNSC[res_print_WK2WK1_AggNSC$fc == "down", ]))
  
 
 # tiff(filename = "./DEG_genes_WK1_WK2_aggregate_vs_NSC_Venn_diagram.tiff", height = 4, width=6, units = "in", res=300)
 
 # cairo_ps("./DEG_genes_Venn_diagram.eps", height = 4, width = 5, onefile = F)
   venn.plot <- venn.diagram(x=list(S1=as.matrix(sig_int_up), S2=as.matrix(sig_int_down), S3=as.matrix(PD_risk_genes_ens), S4=as.matrix(async_int_proteins_ens_v)), fill = c("red", "cyan3", "green3","yellow"), alpha=c(0.5, 0.5, 0.5,0.5), cex=0.6, cat.cex=0.55, fontfamily="sans", cat.fontfamily="sans", main.fontfamily = "sans", cat.dist=c(0.2,0.2,0.1,0.1),  filename=NULL, euler.d=F, scaled=F, print.mode=c("percent", "raw"), sigdigs = 2, category.names=paste0(c("WK2WK1:AggNSC Up", "WK2WK1:AggNSC Down","PD GWAS genes","a-synuclein interacting proteins"), "\n", c(length(sig_int_up), length(sig_int_down), length(PD_risk_genes_ens), length(async_int_proteins_ens_v))), main = d, main.cex = 0.7)
...
```{r}
cat("the genes which are UP in WK2WK1:AggNSC  & also found to be a-synculein interacting proteins" , fill=T)
#sig_WK2 <- c(sig_WK2_up, sig_WK2_down)
p <- intersect(sig_int_up,async_int_proteins_ens_v)
p_df <- async_int_proteins_ens %>% filter(ens_gene %in% p)
print(p_df$gene) 


cat("the genes which are Down in WK2WK1:AggNSC & also found to be a-synculein interacting proteins" , fill=T)
#sig_WK2 <- c(sig_WK2_up, sig_WK2_down)
p <- intersect(sig_int_down,async_int_proteins_ens_v)
p_df <- async_int_proteins_ens %>% filter(ens_gene %in% p)
print(p_df$gene)
...
```{r, fig.height=6, fig.width=8}
library(EnhancedVolcano)
 Vol <- EnhancedVolcano(resOrdered_WK2WK1_AggNSC_LFCshrink_ens,
    lab =resOrdered_WK2WK1_AggNSC_LFCshrink_ens$symbol,
    x = 'log2FoldChange',
...
resOrdered_WK2WK1_AggNSC_LFCshrink_ens_sig <- resOrdered_WK2WK1_AggNSC_LFCshrink_ens %>% filter(padj < 0.05 ) 

#geneChangeList <- resOrdered_Parent_stim_control_LFCshrink_ens_sig$log2FoldChange
geneChangeList <- unique(rownames(resOrdered_WK2WK1_AggNSC_LFCshrink_ens_sig ))
#geneChangeList <- sort(geneChangeList, decreasing=TRUE)
#desiredGeneChangeList <- names(geneChangeList)[abs(geneChangeList) >1] 
 all_genes <- as.character(unique(rownames(resOrdered_WK2WK1_AggNSC_LFCshrink_ens)))

enrichGOResult_5 <- enrichGO(gene = geneChangeList, OrgDb = org.Hs.eg.db, keyType = 'ENSEMBL', ont = "BP", pAdjustMethod = "BH", universe=all_genes, readable = T)
enrichGOResult_5_s <- simplify(enrichGOResult_5, cutoff=0.8)
```


### GO BP over-representation test
+ No terms einriched have more than 10 genes


```{r, eval=F}

scorePlot <- RScorePlot(enrichGOResult_5, resOrdered_WK2WK1_AggNSC_LFCshrink_ens, showCategory = 20, textSize = 9, title = "" ) + theme(title = element_text(size=8))+ggtitle("GO BP terms for Treatment_time:Condition interactions")
plot(scorePlot)
```
...
library(ReactomePA)
# only takes entrez id
geneChangeList <- unique(resOrdered_WK2WK1_AggNSC_LFCshrink_ens_sig$entrez)
#geneChangeList <- sort(geneChangeList, decreasing=TRUE)
#desiredGeneChangeList <- names(geneChangeList)[abs(geneChangeList) >1] 
 all_genes <- as.character(unique(resOrdered_WK2WK1_AggNSC_LFCshrink_ens$entrez))
enrich_Reactome_5 <-  enrichPathway(gene = geneChangeList, pAdjustMethod = "BH", universe=all_genes, readable = T, organism = "human")
scorePlot <- RScorePlot(enrich_Reactome_5, resOrdered_WK2WK1_AggNSC_LFCshrink_ens, showCategory = 20, textSize = 9, title = "" ) + theme(title = element_text(size=8))+ ggtitle("Reactome terms for Treatment_time:Condition interactions")
plot(scorePlot)
```
...
#desiredGeneChangeList <- names(geneChangeList)[abs(geneChangeList) >1] 
 ...
 enrich_msig_hpo_5 <-  enricher(gene = geneChangeList, pAdjustMethod = "BH", universe=all_genes, TERM2GENE = m_hpo)



scorePlot <- RScorePlot_v3(enrich_msig_hpo_5, resOrdered_WK2WK1_AggNSC_LFCshrink_ens, showCategory = 10, textSize = 9, title = "" ) + theme(title = element_text(size=8), axis.text.y = element_text(size=9)) +ggtitle("HPO genesets for Treatment_time:Condition interactions")
plot(scorePlot)
```
...
#desiredGeneChangeList <- names(geneChangeList)[abs(geneChangeList) >1] 
 ...
 enrich_msig_c3_5 <-  enricher(gene = geneChangeList, pAdjustMethod = "BH", universe=all_genes, TERM2GENE = m_t2g)



scorePlot <- RScorePlot_v3(enrich_msig_c3_5, resOrdered_WK2WK1_AggNSC_LFCshrink_ens, showCategory = 10, textSize = 9, title = "" ) + theme(title = element_text(size=8), axis.text.y = element_text(size=9)) +ggtitle(" Trascription factor genesets for Treatment_time:Condition interactions")
plot(scorePlot)
```
...
```{r,echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE}
library(fgsea)
gsea_res_WK2WK1_AggNSC <- resOrdered_WK2WK1_AggNSC_LFCshrink_ens %>% dplyr::select(symbol,log2FoldChange) %>% na.omit() %>% distinct() %>% group_by(symbol) %>% summarize(log2FoldChange=mean(log2FoldChange))
# dim(gsea_res)
# head(gsea_res)

# fast Gene Set Enrichment Analysis
ranks <- deframe(gsea_res_WK2WK1_AggNSC)
...
# Functional links in PPI for 6 to PD Risk Genes

+ We already know that WK2 vs WK1 (NSC & Aggregate) comparisons have signficiant PD risk genes that are dysregulated

+  To test if DEG genes  Aggregate vs NSC (WK1 & WK2) and the genes DEG in Agg vs NSC and WK2 vs WK1 interaction have higher than expected direct links with PD risk genes in PPIs in humans
+ Controlling fr CDS length for genes and PPI degree
+  PD risk genes are from META5 PD risk  (NALLS 2019) anc Chang 2017 study
...
+  150 unique PD risk genes based on ENS ID
...
## WK1 Aggregate vs NSC

```{r, eval=T}

all_genes <- rownames(resOrdered_WK1_Agg_NSC_LFCshrink_ens)
WK1_Agg_NSC <- c(sig_WK1_up,sig_WK1_down)
```

```{r, eval=T}

WK1_Agg_NSC_pdrisk_ppi <- test_enrich_two_sets_avi(input_net = ppi_net ,input_cds = cds_gene,input_list_1 = WK1_Agg_NSC , input_list_2 =PD_risk_genes_ens  ,input_universe = all_genes,n_rand = 10000 )
```


## WK2 Aggregate vs NSC

```{r, eval=T}

all_genes <- rownames(resOrdered_WK2_Agg_NSC_LFCshrink_ens)
WK2_Agg_NSC <- c(sig_WK2_up,sig_WK2_down)
```

```{r, eval=T}

WK2_Agg_NSC_pdrisk_ppi <- test_enrich_two_sets_avi(input_net = ppi_net ,input_cds = cds_gene,input_list_1 = WK2_Agg_NSC , input_list_2 =PD_risk_genes_ens  ,input_universe = all_genes,n_rand = 10000 )
```



## DEG for Treatment_time:Stimulation


```{r, eval=T}

all_genes <- rownames(resOrdered_WK2WK1_AggNSC_LFCshrink_ens)
WK2WK1_AggNSC <- rownames(resOrdered_WK2WK1_AggNSC_LFCshrink_ens_sig)
```

```{r, eval=T}

WK2_Agg_NSC_pdrisk_ppi <- test_enrich_two_sets_avi(input_net = ppi_net ,input_cds = cds_gene,input_list_1 = WK2WK1_AggNSC , input_list_2 =PD_risk_genes_ens  ,input_universe = all_genes,n_rand = 10000 )
```


## WK2 Aggregate vs WK1 Aggregate
...
WK1_Agg_NSC_asyn_ppi <- test_enrich_two_sets_avi(input_net = ppi_net ,input_cds = cds_gene,input_list_1 = WK1_Agg_NSC , input_list_2 =async_int_proteins_ens_v  ,input_universe = all_genes,n_rand = 10000 )
```
...
WK2_Agg_NSC_async_ppi <- test_enrich_two_sets_avi(input_net = ppi_net ,input_cds = cds_gene,input_list_1 = WK2_Agg_NSC , input_list_2 = async_int_proteins_ens_v ,input_universe = all_genes,n_rand = 10000 )
```
...
WK2WK1_AggNSC_asyn_ppi <- test_enrich_two_sets_avi(input_net = ppi_net ,input_cds = cds_gene,input_list_1 = WK2WK1_AggNSC , input_list_2 = async_int_proteins_ens_v  ,input_universe = all_genes,n_rand = 10000 )
```
...
# Functional links in PPI for PD GWAS genes to a-synuclein interacting proteins
...
PDgwas_asyn_ppi <- test_enrich_two_sets_avi(input_net = ppi_net ,input_cds = cds_gene,input_list_1 = PD_risk_genes_ens , input_list_2 = async_int_proteins_ens_v  ,input_universe = all_genes,n_rand = 10000 )
```
